...
const BookingForm = ({ onBookingCreated }) => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        // Nur aktive Kategorien für neue Buchungen laden
        const response = await apiRequest('/api/categories?active_only=true');
  ...
  useEffect(() => {
    ...
    const fetchProducts = async (platformKey) => {
      ...
      try{
        const res = await apiRequest(`/api/products?platformKey=${encodeURIComponent(platformKey)}&active_only=true`)
